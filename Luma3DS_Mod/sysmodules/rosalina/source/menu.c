...
// sponge (terry davis taught me this, search "sponge" in the project and its probably modifications i made)
extern bool rosalinaMod_ShouldOpenCustomMenu; // added by ctn Firmware from source/ctn/base_menu.c
extern void ctn_MenuManager(void); // added by ctn Firmware from source/ctn/base_menu.c
...
void menuThreadMain(void)
{
    ...
    while(!preTerminationRequested)
    {
        ...
        // START Added by ctn Firmware
        // sponge
        if (rosalinaMod_ShouldOpenCustomMenu){
            menuEnter();
            if(isN3DS) N3DSMenu_UpdateStatus();
            PluginLoader__UpdateMenu();
            menuShow(&rosalinaMenu);
...
static void menuDraw(Menu *menu, u32 selected)
{
    ...
    if(isRelease)
        Draw_DrawFormattedString(10, SCREEN_BOT_HEIGHT - 20, COLOR_TITLE, "Luma3DS %s, (ctn)", versionString); // ctn: added ", (ctn)"
    else
...
void menuShow(Menu *root)
{
    ...
    do
    {
        ...
        if (rosalinaMod_ShouldOpenCustomMenu){
            for (u32 i = 0; i < 24; i++) {
                void *method = currentMenu->items[i].method;
                if (method == ctn_MenuManager){
                    selectedItem = i; // set selected menu
                    pressed |= KEY_A; // simulate A press
                }
            }
        }
        // END
