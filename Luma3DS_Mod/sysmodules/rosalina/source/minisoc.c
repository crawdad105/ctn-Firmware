...
// Added by ctn Firmware (i dont know what im doing)
// https://www.3dbrew.org/wiki/SOCU:fcntl
// this is probably not standard
// according to the wiki the cmd can only be 3 or 4 so im simplifying it to a bool
// good luck for "arg" your on your own if you need the "..." from the original functions
int fcntl(int sockfd, bool set, int arg){

    int ret = 0;
    u32 *cmdbuf = getThreadCommandBuffer();

    cmdbuf[0] = IPC_MakeHeader(0x2,3,2); // 0x200C2
    cmdbuf[1] = (u32)sockfd; // Socket descriptor
    cmdbuf[2] = (u32)(set ? F_GETFL : F_SETFL); // "int cmd"
    cmdbuf[3] = (u32)arg; // "arg"
    cmdbuf[4] = IPC_Desc_CurProcessId(); // same as SOCU:socket so im using that "Value 0x20 (processID header)"
    // not setting cmdbuf[5] because nothing else does
